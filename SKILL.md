...
- **律所拥有者（顶层律所 org + OWNER 角色，需服务端开启全所范围 `pat.firm_scope_enabled`）**：读端点可见范围自动展开为「顶层律所 + 所有直接子团队 org」（扁平一层）。即律所拥有者的 AI Agent 可跨子团队看全所案件/客户/项目/财务/合同/日程/记录等读端点；**写端点（创建/更新/上传）不跨子团队**，仍按 PAT 主 org_id 归属。非 owner 的 PAT 行为零变化。组织结构目前仅支持扁平一层，多级团队递归为二期。

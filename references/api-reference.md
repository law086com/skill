...
|------|---------|
| 个人空间本人 | 全部个人数据 |
| 团队/律所管理员或拥有者 | 本 org 下全部数据；其中团队拥有者/管理员可修改本 org 团队成员的日程 |
...
> - **普通成员**（非管理员/拥有者、非律所顶层 owner）PAT 行为零变化：读只看本 org/自己参与；写端点传非自身 org 会报错 `"目标组织不在本所范围"`。
...
> **不可更新字段**：`status`、`host`/`assit`（律师团队）、`fee_type` 等属独立业务流程，需在 OA 网页端操作，Open API 不开放。这些字段即使传入也会被忽略。
>
> **结案 / 归档不走本端点**：`g_status` / `j_time` / `ja_status` / `g_time` / `g_user` / `g_address` 也不能通过 `PATCH /cases/{code}` 传入（传入会被忽略）。结案、归档及其取消有**独立端点**：`POST /cases/{code}/close` 与 `POST /cases/{code}/archive`（见上文），且当律所开启对应审批流程时这两个端点会被拒绝。
>
...
### POST /cases/{code}/close - 案件结案 / 取消结案

Scope: `cases.close` | **高危操作，需二次确认**

把案件标记为**已结案**，或**取消结案**。案件状态不存独立字段，而是由 `j_time`（结案时间）派生：`j_time` 非空即「已结案」（`g_time` 非空则为「已归档」，优先级更高）。

> **律所开启结案审批时本端点不可用**：若案件所属律所开启了「结案审批」，调用被直接拒绝（报错见下表），须引导用户到 OA 网页端提交结案申请走审批流程。AI **不代提交审批单**，后端也不做部分放行。
>
> **权限范围**：与 `PATCH /cases/{code}` 同口径 —— 律所拥有者（owner）的 PAT 可操作全所（顶层律所 + 直接子团队）任意案件；非 owner 只能操作本 org 且自己参与的案件。个人空间与团队版无审批开关，始终可用。

**参数**（均可选；三字段各自独立解析）:

| 参数 | 类型 | 未提供 | 显式传 `null` 或 `""` | 合法值 |
|------|------|--------|----------------------|--------|
| j_time | string | 案件未结案 → 取**当天**；已结案 → 保持原值 | **清空 → 取消结案** | `YYYY-MM-DD` |
| ja_status | int | 不改动 | 置为 `0`（其它） | `0`~`5` |
| suc_amount | number | 不改动 | 清空 | 数值且 `>= 0`（`0` 是合法值） |
...
> **「未提供」≠「显式清空」**：不传该参数 = 不动它；传 `null` 或 `""` = 清空它。**取消结案 = 显式传 `"j_time": null`**。
>
> ⚠️ **不要把空字符串当作「占位」传进来** —— `"j_time": ""` 等于取消结案。只想改结案状态/胜诉金额时，**不要传 `j_time` 键**。

**取消结案的前置条件**:

- 案件已归档（`g_time` 非空）→ 报错 `案件已归档，请先取消归档再取消结案`（先调 `/archive` 取消归档）
- 案件本就未结案 → 报错 `案件当前未结案，无需取消结案`

**报错**:

| 场景 | msg |
|------|-----|
| 案件不存在 / 无权操作 | `案件不存在或无权操作` |
| 律所已开启结案审批 | `该律所已开启结案审批，无法通过 AI 助手结案。请在案件云 OA 端提交结案申请，走审批流程。` |
| j_time 格式非法 | `结案日期格式无效，请使用 YYYY-MM-DD` |
| ja_status 越界 | `结案状态值无效，合法值为 0~5` |
| suc_amount 非法 | `胜诉金额必须是不小于 0 的数值` |
| 取消结案但案件已归档 | `案件已归档，请先取消归档再取消结案` |
| 取消结案但案件未结案 | `案件当前未结案，无需取消结案` |
| 最终无任何字段实际变更（如对已结案案件传空 body） | `没有可更新的字段` |

**示例请求**:

```bash
POST /cases/aBcDeFgH12345678/close
{
  "j_time": "2026-09-21",
  "ja_status": 1,
  "suc_amount": 10000
...
**响应**: 返回结案后的案件完整数据（同 `GET /cases/{code}` 的 data 结构），可直接读取 `g_status` / `g_status_text` / `j_time` / `ja_status` 确认结果。

> **AI 操作不会出现在案件「动态」时间线**：open API 的操作记在 Open API 专属日志中，不写 OA 的 `sr_oplogs`，因此案件详情页的「动态」看不到 AI 的结案记录。若用户需要留痕在动态里，请引导其在 OA 端操作。

### POST /cases/{code}/archive - 案件归档 / 取消归档

Scope: `cases.archive` | **高危操作，需二次确认**

把案件标记为**已归档**，或**取消归档**。归档由 `g_time`（归档时间）派生：`g_time` 非空即「已归档」。

> **律所开启归档审批时本端点不可用**：同 `/close`，被直接拒绝，须引导用户到 OA 提交归档申请。
>
> **归档不要求先结案**（与 OA 一致）：未结案案件可直接归档，此时 `j_time` 为空、`g_time` 非空，状态显示为「归档」。
>
> **归档审批与结案审批是两个独立开关**：律所可能只开了其中一个，另一个端点仍可用。

**参数**（均可选；三字段各自独立解析）:

| 参数 | 类型 | 未提供 | 显式传 `null` 或 `""` | 合法值 |
|------|------|--------|----------------------|--------|
| g_time | string | 案件未归档 → 取**当天**；已归档 → 保持原值 | **清空 → 取消归档** | `YYYY-MM-DD` |
| g_user | int | 案件 `g_user` 为空 → 取**调用者 uid**；已有值 → 保持原值 | 清空 | 正整数（uid） |
| g_address | string | 不改动 | 清空为 `""` | 最长 128 字 |

> **取消归档会同时清空三个字段**：显式传 `"g_time": null` 时，`g_time`、`g_user`、`g_address` **一并清空**（与 OA 删除归档申请的回滚口径一致），避免留下孤立的归档人/保管地。

**取消归档的前置条件**: 案件本就未归档 → 报错 `案件当前未归档，无需取消归档`。
...
|------|-----|
| 案件不存在 / 无权操作 | `案件不存在或无权操作` |
| 律所已开启归档审批 | `该律所已开启归档审批，无法通过 AI 助手归档。请在案件云 OA 端提交归档申请，走审批流程。` |
| g_time 格式非法 | `归档日期格式无效，请使用 YYYY-MM-DD` |
| g_user 非法 | `归档人无效` |
| g_address 超长 | `档案保管地长度不能超过128字` |
| 取消归档但案件未归档 | `案件当前未归档，无需取消归档` |
| 最终无任何字段实际变更 | `没有可更新的字段` |

**示例请求**:

```bash
POST /cases/aBcDeFgH12345678/archive
{
  "g_time": "2026-09-21",
  "g_address": "档案室 A-03"
}
...
**响应**: 返回归档后的案件完整数据（同 `GET /cases/{code}` 的 data 结构）。

> ⚠️ **新 scope 需重新生成 PAT**：`cases.close` / `cases.archive` 是本次新增的权限项。**已有 PAT 令牌不包含它们**，调用这两个端点会返回 scope 鉴权失败。需到 OA「个人设置 > AI管理」编辑令牌勾选新权限，或重新生成令牌。
...
| huser | string | 否 | 主办人 UID（hashid，单人，来自 GET /team/members）。⚠️ **团队/律所管理员或拥有者可用**（含律所顶层 OWNER 全所范围），用于"派活/指派主办"场景；非管理员/拥有者传入会被忽略（主办人恒为调用者本人）。详见下方「主办人指派规则」 |
...
`POST /calendar` 创建待办/办案记录时，**主办人（`huser`）**默认 = 调用者本人。团队/律所管理员或拥有者（PAT 绑定本 org，含律所顶层 OWNER 全所范围）可指定主办人，使被指定的员工成为待办主办并能在「我的待办」中看到。确定优先级（管理员/拥有者身份下）：
...
| 身份/场景 | `huser` 取值 | 说明 |
|-----------|--------------|------|
| 管理员/拥有者 + 传 `huser` | 传入值（解码+校验后） | 精确指定主办，**推荐用于派活场景**（与 `PUT /calendar/{id}` 行为一致） |
| 管理员/拥有者 + 未传 `huser` + 传 `assit` | `assit` 第一个员工 | 兜底场景，多人协办时取第一个 |
| 管理员/拥有者 + 都没传 | 调用者本人 | 兼容现状 |
| 非管理员/拥有者（普通成员） | 调用者本人（**忽略 `huser` 参数**） | 即使传了 `huser` 也会被丢弃 |
| 个人空间（`org_id=0`） | 调用者本人 | 不允许指定他人 |

> **说明**：
> - `add_uid`（创建人）始终 = 调用者，记录"谁派发的"，不受 `huser` 影响。
> - 管理员/拥有者派活后**不**把指派者加入 `assit`，仅 `huser=员工`（指派者靠管理员/全所视图查看该待办）。
...
> **律所拥有者可跨子团队编辑**：owner 的 PAT 可更新全所任意日程/记录（保留记录自身 org_id）；成员校验按记录自身 org。
>
> **团队/律所管理员或拥有者**：团队拥有者/管理员可更新本 org 任意日程/记录，不限于本人主办或协办；普通成员仍只能更新本 org 且自己参与（主办/协办）的日程。
>
...
| cases.close | 案件结案 / 取消结案 | POST /cases/{code}/close |
| cases.archive | 案件归档 / 取消归档 | POST /cases/{code}/archive |
